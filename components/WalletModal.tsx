...
import { X, Wallet, Gem, Coins, ArrowRightLeft, AlertCircle, TrendingUp } from 'lucide-react';
...
interface WalletModalProps {
  ...
  onClose: () => void;
  user: User;
  onExchange: (diamonds: number) => Promise<void>;
}
      ...
      >
        ...
        <div className="p-6 grid grid-cols-2 gap-3">
          <div className="bg-slate-900/50 border border-white/5 p-4 rounded-3xl text-center">
            ...
            <div className="text-xl font-black text-white">{(user.coins || 0).toLocaleString()}</div>
          </div>
          <div className="bg-slate-900/50 border border-white/5 p-4 rounded-3xl text-center">
            ...
            <div className="text-xl font-black text-white">{(user.diamonds || 0).toLocaleString()}</div>
          </div>
        </div>
        ...
        <div className="px-6 pb-8">
          <div className="bg-slate-900 rounded-[2rem] p-5 border border-white/5">
            ...
            <div className="space-y-4">
              ...
              <button 
                disabled={loading || !exchangeAmount || Number(exchangeAmount) <= 0 || Number(exchangeAmount) > currentDiamonds}
                onClick={handleExchange}
                className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 py-4 rounded-2xl text-white font-black text-sm shadow-xl active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : <><TrendingUp size={18}/> تأكيد الاستبدال</>}
              </button>
            </div>
          </div>
          ...
          <div className="mt-4 flex items-start gap-2 px-2">
            ...
            <p className="text-[9px] text-slate-500 leading-relaxed text-right">
              الألماس يتم الحصول عليه من خلال الهدايا التي تتلقاها في الغرف. يمكنك تحويله إلى كوينز لإعادة استخدامه في المتجر أو دعم الآخرين.
            </p>
          </div>
        </div>
      </motion.div>
